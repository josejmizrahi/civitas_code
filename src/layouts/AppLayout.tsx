    ...
    <div className="flex h-dvh flex-col overflow-x-hidden lg:flex-row">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t bg-background/95 backdrop-blur-sm pb-safe lg:hidden">
        <div className="flex items-stretch justify-around">
          {BOTTOM_NAV_ITEMS.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.href}
                className={cn(
                  'relative flex flex-1 flex-col items-center justify-center gap-0.5 min-h-[52px] py-2 px-2 transition-colors',
                  active && 'bg-primary/5'
                )}
              >
                {active && (
          ...
          <button
            onClick={() => setMobileSidebarOpen(true)}
            className="relative flex flex-1 flex-col items-center justify-center gap-0.5 min-h-[52px] py-2 px-2"
          >
